...
@app.route('/virus/{virus_id}', methods=['DELETE'],cors=cors_config)
...
def send_message(event:DynamoDBEvent):
    ...
    for event_received in event:
        if event_received.event_name=='INSERT':
            ...
            new_item = {k: deserializer.deserialize(v) for k, v in event_received.new_image.items()}
...
def spread_virus(event):
    response = client.put_events(Entries=[{'EventBusName':'eventBus',
    'Source':'Time','DetailType':'VIRUS_CREATION_REQUESTED','Detail':json.dumps({})}])
    print (response)
